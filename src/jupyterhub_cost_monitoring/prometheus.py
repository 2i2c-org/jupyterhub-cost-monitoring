...
class Prometheus(LoggingConfigurable):
    ...
    async def query(self, query: str, date_range: DateRange, step: str) -> dict:
        """
        ...
        if prometheus_username != "" and prometheus_password != "":
            prometheus_auth = aiohttp.BasicAuth(
        ...
        query_api = prometheus_api.with_path("/api/v1/query_range").with_query(
            parameters
        )
        async with self.client_session.get(query_api, auth=prometheus_auth) as response:
    ...
    async def query_user_groups(
        ...
        response = await self.query(USER_GROUP_INFO, date_range, step="1d")
    ...
    async def query_users_with_multiple_groups(
        ...
        response = await self.query_user_groups(hub_name=hub_name, user_name=user_name)
        grouped = defaultdict(
            lambda: {
                "username": None,
                "hub": None,
